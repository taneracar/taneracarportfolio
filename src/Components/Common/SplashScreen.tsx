...
const SplashScreen = () => {
  ...
  useEffect(() => {
    ...
    const paths = svgRef.current?.querySelectorAll(".spark");
    if (paths) {
  ...
  return (
    <div
      ref={containerRef}
      className="fixed inset-0 z-[9999] bg-black flex items-center justify-center gap-[3vw]"
    >
      <div
        ref={textRef}
      ...
      <div ref={svgRef} className="opacity-0">
        <svg
          ...
          aria-hidden="true"
          role="img"
          className="iconify iconify--emojione-monotone w-[clamp(2rem,6vw,8rem)] h-[clamp(2rem,6vw,8rem)]"
